    ...
    return (
      <Container>
        <Carousel autoPlay emulateTouch infiniteLoop>
          {DUMMY_DATA.results.map(({ id, poster_path, original_title, release_date, vote_average, overview }) => (
            <div key={id}>
              <Image src="/logo192.png" alt="error"></Image>
              <Paragraph>
                <h1>{original_title}</h1>
                <div>{release_date + " / " + vote_average + "★"}</div>
                <p>{overview}</p>
              </Paragraph>
            </div>
          ))}
